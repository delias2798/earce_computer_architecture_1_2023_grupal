...
A = cv2.imread('barbara.jpg', cv2.IMREAD_GRAYSCALE)
...
for k in range(5, 200, 5):
    ...
    for x in range(m):
        for y in range(n):
            # Calcular las nuevas coordenadas para el pixel actual
            ...
            # Validar los rangos de la imagen
            if (xnew == xaux) and (ynew == yaux):
                B[xnew, ynew] = A[x, y]
    plt.subplot(1, 2, 2)
    plt.imshow(B, cmap='gray')
    # Delay por un corto periodo de tiempo
    plt.pause(0.01)
    Y[:, :, k // 5] = B